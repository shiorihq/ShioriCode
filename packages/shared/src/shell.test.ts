...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__SHIORICODE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__SHIORICODE_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__SHIORICODE_ENV_PATH_START__\n/a:/b\n__SHIORICODE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__SHIORICODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__SHIORICODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__SHIORICODE_ENV_PATH_END__",
        "__SHIORICODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__SHIORICODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__SHIORICODE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__SHIORICODE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
