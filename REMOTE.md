...
Use this when you want to open ShioriCode from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The ShioriCode CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var                   | Notes                                                                                |
| ----------------------- | ------------------------- | ------------------------------------------------------------------------------------ |
| `--mode <web\|desktop>` | `SHIORICODE_MODE`         | Runtime mode.                                                                        |
| `--port <number>`       | `SHIORICODE_PORT`         | HTTP/WebSocket port.                                                                 |
| `--host <address>`      | `SHIORICODE_HOST`         | Bind interface/address.                                                              |
| `--base-dir <path>`     | `SHIORICODE_HOME`         | Base directory.                                                                      |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL`     | Dev web URL redirect/proxy target.                                                   |
| `--no-browser`          | `SHIORICODE_NO_BROWSER`   | Disable auto-open browser.                                                           |
| `--auth-token <token>`  | `SHIORICODE_AUTH_TOKEN`   | WebSocket auth token. Use this for standard CLI and remote-server flows.             |
| `--bootstrap-fd <fd>`   | `SHIORICODE_BOOTSTRAP_FD` | Read a one-shot bootstrap envelope from an inherited file descriptor during startup. |
