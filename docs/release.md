...
- Publishes the CLI package (`apps/server`, npm package `shioricode`) with OIDC trusted publishing.
...
1. Confirm npm org/user owns package `shioricode` (or rename package first if needed).
