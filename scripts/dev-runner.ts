...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("SHIORICODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("SHIORICODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid SHIORICODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `SHIORICODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric SHIORICODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed SHIORICODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_SHIORICODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly shioriCodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(shioriCodeHome);
    ...
    if (!isDesktopMode) {
      output.SHIORICODE_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.SHIORICODE_PORT;
      delete output.VITE_WS_URL;
      delete output.SHIORICODE_AUTH_TOKEN;
      delete output.SHIORICODE_MODE;
      delete output.SHIORICODE_NO_BROWSER;
      delete output.SHIORICODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.SHIORICODE_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.SHIORICODE_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.SHIORICODE_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.SHIORICODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.SHIORICODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.SHIORICODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.SHIORICODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.SHIORICODE_MODE = "web";
      delete output.SHIORICODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read SHIORICODE_PORT_OFFSET/SHIORICODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("SHIORICODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("SHIORICODE_LOG_WS_EVENTS"),
    };
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      shioriCodeHome: input.shioriCodeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SHIORICODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.SHIORICODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  shioriCodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all ShioriCode data (equivalent to SHIORICODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("SHIORICODE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to SHIORICODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("SHIORICODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to SHIORICODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("SHIORICODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to SHIORICODE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("SHIORICODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to SHIORICODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("SHIORICODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to SHIORICODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("SHIORICODE_PORT")),
  ),
