...
interface StagePackageJson {
  ...
  readonly shioriCodeCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "SHIORICODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("SHIORICODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "SHIORICODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("SHIORICODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("SHIORICODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("SHIORICODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("SHIORICODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("SHIORICODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("SHIORICODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("SHIORICODE_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("SHIORICODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
    Config.option,
  ),
});
...
function resolveGenericPublishConfig():
  | { readonly provider: "generic"; readonly url: string }
  | undefined {
  const url = process.env.SHIORICODE_DESKTOP_UPDATE_URL?.trim();
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.SHIORICODE_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.shioritools.shioricode",
    productName,
    artifactName: "ShioriCode-${version}-${arch}.${ext}",
    directories: {
  ...
  } else if (genericPublishConfig) {
    buildConfig.publish = [genericPublishConfig];
  } else if (githubPublishConfig) {
    buildConfig.publish = [githubPublishConfig];
  }
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "shioricode",
      icon: "icon.png",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "shioricode",
        },
      },
    };
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `shioricode-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    shioriCodeCommitHash: commitHash,
    private: true,
    description: "ShioriCode desktop build",
    author: "Shiori Tools",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "ShioriCode",
      options.signed,
      options.mockUpdates,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: SHIORICODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: SHIORICODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription(
      "Build arch, for example arm64/x64/universal (env: SHIORICODE_DESKTOP_ARCH).",
    ),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: SHIORICODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: SHIORICODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: SHIORICODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: SHIORICODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: SHIORICODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: SHIORICODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: SHIORICODE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription(
      "Mock update server port (env: SHIORICODE_DESKTOP_MOCK_UPDATE_SERVER_PORT).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for ShioriCode."),
