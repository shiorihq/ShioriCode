...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit SHIORICODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "SHIORICODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid SHIORICODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults SHIORICODE_HOME to ~/.shiori when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHIORICODE_HOME, resolve(homedir(), ".shiori"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          shioriCodeHome: "/tmp/custom-shiori",
          authToken: "secret",
          noBrowser: true,
        ...
        assert.equal(env.SHIORICODE_HOME, resolve("/tmp/custom-shiori"));
        assert.equal(env.SHIORICODE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.SHIORICODE_NO_BROWSER, "1");
        assert.equal(env.SHIORICODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.SHIORICODE_LOG_WS_EVENTS, "1");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            SHIORICODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          shioriCodeHome: undefined,
        ...
        assert.equal(env.SHIORICODE_MODE, "web");
        assert.equal(env.SHIORICODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHIORICODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom shioriCodeHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.SHIORICODE_HOME, resolve("/tmp/my-shiori"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: {
            SHIORICODE_PORT: "3773",
            SHIORICODE_AUTH_TOKEN: "stale-token",
            SHIORICODE_MODE: "web",
            SHIORICODE_NO_BROWSER: "0",
        ...
        assert.equal(env.SHIORICODE_NO_BROWSER, undefined);
        assert.equal(env.SHIORICODE_HOST, undefined);
