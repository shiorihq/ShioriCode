...
const LINUX_WM_CLASS = isDevelopment ? "shioricode-dev" : "shioricode";
const USER_DATA_DIR_NAME = isDevelopment ? "shioricode-dev" : "shioricode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "ShioriCode (Dev)" : "ShioriCode (Alpha)";
...
function companionCliInstallCommand(packageManager: "bun" | "npm" | "pnpm"): string {
  switch (packageManager) {
    case "bun":
      return `bun add --global ${COMPANION_CLI_PACKAGE_NAME}`;
    case "pnpm":
      return `pnpm add --global ${COMPANION_CLI_PACKAGE_NAME}`;
    case "npm":
    default:
      return `npm install --global ${COMPANION_CLI_PACKAGE_NAME}`;
  }
}

async function detectCompanionCliPackageManager(): Promise<"bun" | "npm" | "pnpm" | null> {
  if (process.platform === "win32") {
    for (const candidate of ["bun", "npm", "pnpm"] as const) {
  ...
  const result = await runCommandInLoginShell(
    shell,
    "command -v bun >/dev/null 2>&1 && echo bun || command -v npm >/dev/null 2>&1 && echo npm || command -v pnpm >/dev/null 2>&1 && echo pnpm",
  );
  if (result.code !== 0) {
    return null;
  }
  const detected = result.stdout.trim();
  if (detected === "bun" || detected === "npm" || detected === "pnpm") {
    return detected;
  }
  return null;
}

function parseCompanionCliVersion(output: string): string | null {
  ...
  const match = firstLine.match(/v?(\d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?)/);
...
function parseFcListFontFamilies(output: string): string[] {
  return output
    .split(/\r?\n/)
...
function parseAtsutilFontFamilies(output: string): string[] {
  return output
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.length > 0 && !line.endsWith("Fonts:"));
}

function parseWindowsRegistryFontFamilies(output: string): string[] {
  return output
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
...
function listSystemFontsSync(): string[] {
  if (systemFontListCache) {
    return systemFontListCache;
  }

  let fontFamilies: string[] = [];

  if (process.platform === "darwin") {
    const fcListOutput = readCommandOutput("fc-list", [":", "family"]);
    fontFamilies = parseFcListFontFamilies(fcListOutput);
    if (fontFamilies.length === 0) {
      fontFamilies = parseAtsutilFontFamilies(readCommandOutput("atsutil", ["fonts", "-list"]));
    }
  } else if (process.platform === "linux") {
    fontFamilies = parseFcListFontFamilies(readCommandOutput("fc-list", [":", "family"]));
  } else if (process.platform === "win32") {
    const powershellCommand =
      "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; " +
      "(Get-ItemProperty 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts' | " +
      "Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name)";
    fontFamilies = parseWindowsRegistryFontFamilies(
      readCommandOutput("powershell.exe", ["-NoProfile", "-Command", powershellCommand]),
    );
  }

  systemFontListCache = dedupeFontFamilies(fontFamilies);
  return systemFontListCache;
}

async function detectInstalledCompanionCli(): Promise<{
  version: string | null;
  binaryPath: string | null;
}> {
  if (process.platform === "win32") {
    const whereResult = ChildProcess.spawnSync("where", [COMPANION_CLI_BINARY_NAME], {
      shell: true,
      encoding: "utf8",
    });
    const binaryPath =
    ...
    const versionResult = ChildProcess.spawnSync(COMPANION_CLI_BINARY_NAME, ["--version"], {
      shell: true,
      encoding: "utf8",
    });
    return {
      version:
        versionResult.status === 0
          ? parseCompanionCliVersion(`${versionResult.stdout}\n${versionResult.stderr}`)
          : null,
      binaryPath,
    };
  }
  ...
  const binaryMarker = "__SHIORI_BINARY__";
  const result = await runCommandInLoginShell(
    shell,
    [
      `binary_path="$(command -v ${quoteShellArg(COMPANION_CLI_BINARY_NAME)} 2>/dev/null)"`,
      'if [ -z "$binary_path" ]; then exit 127; fi',
      `printf '%s%s\\n' '${binaryMarker}' "$binary_path"`,
      `${quoteShellArg(COMPANION_CLI_BINARY_NAME)} --version`,
    ].join("; "),
  );
  ...
  const binaryPath =
    result.stdout
      .split(/\r?\n/)
      .map((line) => line.trim())
      .find((line) => line.startsWith(binaryMarker))
      ?.slice(binaryMarker.length) ?? null;
  const version = parseCompanionCliVersion(
    result.stdout
      .split(/\r?\n/)
      .filter((line) => !line.startsWith(binaryMarker))
      .join("\n"),
  );
...
async function refreshCompanionCliState(
  overrides: Partial<DesktopCompanionCliState> = {},
): Promise<DesktopCompanionCliState> {
  const packageManager = await detectCompanionCliPackageManager();
  const installCommand = companionCliInstallCommand(packageManager ?? "npm");
  const detected = await detectInstalledCompanionCli();

  companionCliState = {
    status: detected.binaryPath ? "installed" : overrides.lastError ? "error" : "not-installed",
    version: detected.version,
    binaryPath: detected.binaryPath,
    lastError: overrides.lastError ?? companionCliState.lastError,
    installCommand,
    ...overrides,
  };

  if (companionCliState.status === "installed") {
    companionCliState = {
      ...companionCliState,
  ...
  return companionCliState;
}

async function installCompanionCli(): Promise<DesktopCompanionCliInstallResult> {
  if (companionCliInstallInFlight) {
    return {
      accepted: false,
      completed: false,
      state: { ...companionCliState, status: "installing" },
    };
  }
  ...
  if (!packageManager) {
    return {
      accepted: false,
      completed: false,
      state: await refreshCompanionCliState({
        status: "error",
        lastError:
          "No supported package manager found. Install the CLI manually with the shown command.",
        installCommand,
      }),
    };
  }

  companionCliInstallInFlight = true;
  companionCliState = {
    ...companionCliState,
    status: "installing",
    installCommand,
  ...
  try {
      ...
      if (!shell) {
        return {
          ...
          state: await refreshCompanionCliState({
            status: "error",
            installCommand,
            lastError: "Could not resolve a login shell to run the install command.",
          }),
        };
      }
      result = await runCommandInLoginShell(shell, installCommand, {
        timeoutMs: 120_000,
      });
    }
    ...
    if (result.code !== 0) {
      return {
        ...
        state: await refreshCompanionCliState({
          status: "error",
          installCommand,
          lastError: (result.stderr || result.stdout || "Install failed.").trim(),
        }),
      };
    }

    const nextState = await refreshCompanionCliState({
      installCommand,
    });
    if (!nextState.binaryPath || !nextState.version) {
      return {
        accepted: true,
        completed: false,
        state: await refreshCompanionCliState({
    ...
    companionCliInstallInFlight = false;
  }
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { shioriCodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.shioriCodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.SHIORICODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("ShioriCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.SHIORICODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `ShioriCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/ShioriCode (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`shioricode`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.SHIORICODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() ||
  ...
  if (process.env.SHIORICODE_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.SHIORICODE_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle(GET_WINDOW_CONTROLS_INSET_CHANNEL, async () =>
    resolveDesktopWindowControlsInset(process.platform),
  );
  ...
  ipcMain.handle(COMPUTER_USE_PERMISSION_GUIDE_CHANNEL, async (_event, rawKind: unknown) => {
    ...
    try {
      return await showComputerUsePermissionGuide(kind);
    } catch (error) {
  ...
  ipcMain.removeHandler(COMPANION_CLI_GET_STATE_CHANNEL);
  ipcMain.handle(COMPANION_CLI_GET_STATE_CHANNEL, async () => await refreshCompanionCliState());

  ipcMain.removeHandler(COMPANION_CLI_INSTALL_CHANNEL);
  ipcMain.handle(COMPANION_CLI_INSTALL_CHANNEL, async () => await installCompanionCli());
}
