...
import { ServerSettings, ServerSettingsPatch, ModelSelection, ThreadEnvMode } from "contracts";
import {
  type ClientSettings,
...
import { deepMerge } from "shared/Struct";
...
import {
  ...
  removeLegacyThemeMode,
  writeStoredClientSettings,
} from "../clientSettings";
...
export function buildLegacyClientSettingsMigrationPatch(
  ...
  if (Predicate.isBoolean(legacySettings.blurPersonalData)) {
    patch.blurPersonalData = legacySettings.blurPersonalData;
  }
...
export function migrateLegacyThemePreference(): void {
  if (typeof window === "undefined") return;
  ...
  const currentSettings = readStoredClientSettings();
  writeStoredClientSettings({
    ...currentSettings,
