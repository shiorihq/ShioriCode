...
describe("estimateTimelineMessageHeight", () => {
  ...
  it("uses narrower width to increase user line wrapping", () => {
    ...
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 320 })).toBe(132);
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 768 })).toBe(110);
  });
  ...
  it("does not clamp user wrapping too aggressively on very narrow layouts", () => {
    ...
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 100 })).toBe(154);
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 320 })).toBe(110);
  });
  ...
  it("uses narrower width to increase assistant line wrapping", () => {
    ...
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 320 })).toBe(146.75);
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 768 })).toBe(78.5);
  });
});
