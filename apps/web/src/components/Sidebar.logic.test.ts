...
import { OrchestrationLatestTurn, ProjectId, ThreadId } from "contracts";
import {
...
describe("getProjectDeletionBlocker", () => {
  it("ignores stale thread ids with no sidebar summary", () => {
    expect(
      getProjectDeletionBlocker({
  ...
  it("keeps the existing message when active threads remain", () => {
    expect(
      getProjectDeletionBlocker({
  ...
  it("explains when only archived threads are blocking project deletion", () => {
    expect(
      getProjectDeletionBlocker({
      ...
      description:
        "This project still has 1 archived thread. Archived threads are hidden in the sidebar. Delete them from Settings > Archived before removing the project.",
    });
  });
});

describe("getProjectThreadCounts", () => {
  it("counts active and archived threads independently", () => {
    expect(
      getProjectThreadCounts({
        threadIds: ["thread-active", "thread-archived", "missing-thread"],
        threadsById: {
          "thread-active": {
            archivedAt: null,
          },
...
describe("getArchivedOnlyProjectEmptyStateLabel", () => {
  it("uses singular wording for one archived thread", () => {
    expect(getArchivedOnlyProjectEmptyStateLabel(1)).toBe("1 archived thread hidden");
  });

  it("uses plural wording for multiple archived threads", () => {
    expect(getArchivedOnlyProjectEmptyStateLabel(2)).toBe("2 archived threads hidden");
  });
});
...
describe("getDefaultSidebarProjectId", () => {
  it("returns the first project in sidebar order after applying sort settings", () => {
    expect(
      getDefaultSidebarProjectId({
        projects: [
          {
        ...
        threads: [
          ...
          },
          {
            ...
            archivedAt: null,
            latestUserMessageAt: "2026-03-09T11:10:00.000Z",
          },
        ],
        preferredProjectIds: [ProjectId.makeUnsafe("project-1"), ProjectId.makeUnsafe("project-2")],
        sortOrder: "updated_at",
      }),
    ).toBe(ProjectId.makeUnsafe("project-2"));
  });

  it("keeps manual project order when manual sorting is selected", () => {
    expect(
      getDefaultSidebarProjectId({
        projects: [
          {
    ...
    ).toBe(ProjectId.makeUnsafe("project-1"));
  });
});

describe("getOrderedActiveSidebarThreadIds", () => {
  it("returns active thread ids in sidebar traversal order across sorted projects", () => {
    expect(
      getOrderedActiveSidebarThreadIds({
        projects: [
          {
        ...
        threadsById: {
          ...
          "thread-3": {
            ...
            updatedAt: "2026-03-09T11:35:00.000Z",
            archivedAt: "2026-03-10T11:35:00.000Z",
            latestUserMessageAt: "2026-03-09T11:35:00.000Z",
          },
        },
        threadIdsByProjectId: {
          "project-1": [ThreadId.makeUnsafe("thread-1")],
          "project-2": [ThreadId.makeUnsafe("thread-2"), ThreadId.makeUnsafe("thread-3")],
        },
        preferredProjectIds: [ProjectId.makeUnsafe("project-1"), ProjectId.makeUnsafe("project-2")],
