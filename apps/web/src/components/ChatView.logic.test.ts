...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("keeps local dispatch alive while a provider session is attached before the turn starts", () => {
    const localDispatch = createLocalDispatchSnapshot({
      id: ThreadId.makeUnsafe("thread-1"),
      codexThreadId: null,
      projectId,
  ...
  it("clears local dispatch when the turn is observed as running", () => {
    const localDispatch = createLocalDispatchSnapshot({
      id: ThreadId.makeUnsafe("thread-1"),
      codexThreadId: null,
      projectId,
  ...
  it("clears local dispatch when provider turn start fails before the turn is observed running", () => {
    const localDispatch = createLocalDispatchSnapshot({
      id: ThreadId.makeUnsafe("thread-1"),
      codexThreadId: null,
      projectId,
    ...
    const failureCreatedAt = new Date(Date.now() + 1_000).toISOString();
