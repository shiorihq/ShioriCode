...
import type { SidebarProjectSortOrder, SidebarThreadSortOrder } from "contracts/settings";
...
export function getOrderedActiveSidebarThreadIds<
  TProject extends SidebarProject,
  TThread extends Pick<Thread, "id" | "projectId" | "createdAt" | "updatedAt" | "archivedAt"> &
    SidebarThreadSortInput,
>(input: {
  projects: readonly TProject[];
  threadsById: Readonly<Record<string, TThread | undefined>>;
  threadIdsByProjectId: Readonly<Record<string, readonly TThread["id"][] | undefined>>;
  preferredProjectIds: readonly TProject["id"][];
  projectSortOrder: SidebarProjectSortOrder;
  threadSortOrder: SidebarThreadSortOrder;
}): TThread["id"][] {
  const orderedProjects = orderItemsByPreferredIds({
    items: input.projects,
    preferredIds: input.preferredProjectIds,
    getId: (project) => project.id,
  });
  const activeThreads = Object.values(input.threadsById).filter(
    (thread): thread is TThread => thread !== undefined && thread.archivedAt === null,
  );
  ...
  return sortedProjects.flatMap((project) =>
    sortThreadsForSidebar(
      (input.threadIdsByProjectId[project.id] ?? [])
        .map((threadId) => input.threadsById[threadId])
        .filter((thread): thread is TThread => thread !== undefined && thread.archivedAt === null),
...
function formatThreadCount(count: number, label: "active" | "archived"): string {
  return `${count} ${label} thread${count === 1 ? "" : "s"}`;
}

export function getProjectThreadCounts(input: {
  threadIds: readonly string[];
  threadsById: Readonly<Record<string, Pick<SidebarThreadSummary, "archivedAt"> | undefined>>;
}): {
...
export function getArchivedOnlyProjectEmptyStateLabel(archivedThreadCount: number): string {
  return `${formatThreadCount(archivedThreadCount, "archived")} hidden`;
}

export function getProjectDeletionBlocker(input: {
  threadIds: readonly string[];
  threadsById: Readonly<Record<string, Pick<SidebarThreadSummary, "archivedAt"> | undefined>>;
}): { title: string; description: string } | null {
  const { activeThreadCount, archivedThreadCount } = getProjectThreadCounts(input);

  if (activeThreadCount === 0 && archivedThreadCount === 0) {
    return null;
  }

  if (archivedThreadCount === 0) {
    return {
  ...
  if (activeThreadCount === 0) {
    return {
      ...
      description: `This project still has ${formatThreadCount(archivedThreadCount, "archived")}. Archived threads are hidden in the sidebar. Delete them from Settings > Archived before removing the project.`,
    };
  }

  return {
    title: "Project has active and archived threads",
    description: `This project still has ${formatThreadCount(activeThreadCount, "active")} and ${formatThreadCount(archivedThreadCount, "archived")}. Delete the active threads, then remove the archived ones from Settings > Archived before removing the project.`,
  };
}
...
export function getDefaultSidebarProjectId<
  TProject extends SidebarProject,
  TThread extends Pick<Thread, "projectId" | "createdAt" | "updatedAt" | "archivedAt"> &
    SidebarThreadSortInput,
>(input: {
  projects: readonly TProject[];
  threads: readonly TThread[];
  ...
  return sortProjectsForSidebar(orderedProjects, visibleThreads, input.sortOrder)[0]?.id ?? null;
}
