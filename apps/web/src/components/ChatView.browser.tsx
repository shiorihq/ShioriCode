...
function createBaseServerConfig(): ServerConfig {
  return {
    cwd: "/repo/project",
    serverInstancePath: "/repo/project/server-instance.json",
    keybindingsConfigPath: "/repo/project/.shioricode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it.skip("opens a new draft for the first sidebar project instead of restoring the bootstrap thread", async () => {
    ...
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-startup-project-order-test" as MessageId,
        targetText: "startup project ordering test",
      }),
      initialEntries: ["/"],
      configureFixture: (nextFixture) => {
        nextFixture.snapshot = {
          ...
          projects: [
            {
              ...nextFixture.snapshot.projects[0]!,
              updatedAt: isoAt(0),
            },
            {
              id: secondProjectId,
          ...
          threads: [
            {
              ...nextFixture.snapshot.threads[0]!,
              updatedAt: isoAt(5),
            },
            {
              id: secondThreadId,
    ...
    try {
      const newThreadPath = await waitForURL(
        ...
        "Route should open a new draft thread for the first sidebar project.",
      );
      ...
      await vi.waitFor(() => {
        expect(
          useComposerDraftStore.getState().draftThreadsByThreadId[newThreadId]?.projectId,
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("shioricode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            cwd: "/repo/project",
            env: {
              SHIORICODE_PROJECT_ROOT: "/repo/project",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              SHIORICODE_PROJECT_ROOT: "/repo/project",
              SHIORICODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/shioricode/pull/1359",
              baseBranch: "main",
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              SHIORICODE_PROJECT_ROOT: "/repo/project",
              SHIORICODE_WORKTREE_PATH: "/repo/worktrees/pr-1359",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("dispatches the active turn id when stopping generation", async () => {
    const baseSnapshot = createSnapshotForTargetUser({
      targetMessageId: "msg-user-stop-command" as MessageId,
      targetText: "stop button dispatch target",
      sessionStatus: "running",
    });
    const targetTurnId = "turn-stop-command" as TurnId;
    const thread = baseSnapshot.threads[0];
    if (!thread) {
      throw new Error("Missing thread fixture for stop dispatch test.");
    }
    const session = thread.session;
    if (!session) {
      throw new Error("Missing thread session fixture for stop dispatch test.");
    }
    const snapshot = {
      ...baseSnapshot,
      threads: [
