...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const ensureHostedShioriAuthTokenSynced = useCallback(async () => {
    const api = readNativeApi();
    if (!api) {
      throw new Error("Native API not found");
    }
    if (selectedProvider !== "shiori") {
      return;
    }
  ...
  }, [hostedShioriAuthToken, selectedProvider]);
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    if (standaloneSlashCommand) {
      if (standaloneSlashCommand === "review") {
        promptForSend = "Review the recent changes and suggest improvements.";
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    await api.orchestration
      ...
      .then(() => {
        return ensureHostedShioriAuthTokenSynced();
      })
      .then(() => {
    ...
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "review") {
          const replacement = "Review the recent changes and suggest improvements.";
          const applied = applyPromptReplacement(
            trigger.rangeStart,
            trigger.rangeEnd,
            replacement,
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          >
            ...
            >
              ...
              <div
                className={cn(
                  "group min-w-0 rounded-[22px] p-px transition-colors duration-200",
