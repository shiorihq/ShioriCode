...
type CommandProps = React.ComponentProps<typeof Autocomplete> & {
  defaultValue?: string;
...
function Command({ autoHighlight = "always", keepHighlight = true, ...props }: CommandProps) {
  return (
    <Autocomplete
...
function CommandPanel({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      // The results panel owns the paper; the popup around it is `clear`, so
      // under a wallpaper the search field floats on the photograph and the
      // list below it reads on a sheet.
