...
const WORKSPACE_DIRECTORY_LIST_MAX_ENTRIES = 500;
      ...
      const listedPaths = [...listedFiles.paths]
        ...
        .filter(
          (entry) =>
            ...
            !isIgnoredEntryName(basenameOf(entry)),
        );
  ...
  const listDirectory: WorkspaceEntriesShape["listDirectory"] = Effect.fn(
    ...
    const candidateEntries = dirents
      ...
      .map((dirent) => ({
        ...
        relativePath:
          target.relativePath === "."
            ? toPosixPath(dirent.name)
            : toPosixPath(path.join(target.relativePath, dirent.name)),
      }))
      .filter((candidate) => !isPathInIgnoredDirectory(candidate.relativePath));
    ...
    const allowedPathSet = shouldFilterWithGitIgnore
      ? new Set(
          yield* filterGitIgnoredPaths(
            normalizedCwd,
            candidateEntries.map((candidate) => candidate.relativePath),
          ),
        )
      : null;
