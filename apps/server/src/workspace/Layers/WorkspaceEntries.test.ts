...
const makeTempDir = Effect.fn(function* (opts?: { prefix?: string; git?: boolean }) {
  ...
  const dir = yield* fileSystem.makeTempDirectoryScoped({
    prefix: opts?.prefix ?? "shioricode-workspace-entries-",
  });
  if (opts?.git) {
...
const listWorkspaceDirectory = (input: { cwd: string; relativePath?: string }) =>
  Effect.gen(function* () {
    const workspaceEntries = yield* WorkspaceEntries;
...
it.layer(TestLayer)("WorkspaceEntriesLive", (it) => {
  ...
  describe("search", () => {
    ...
    it.effect("filters and ranks entries by query", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("supports fuzzy subsequence queries for composer path search", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-fuzzy-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("tracks truncation without sorting every fuzzy match", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-fuzzy-limit-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("excludes gitignored paths for git repositories", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-gitignore-", git: true });
    ...
    it.effect("excludes tracked paths that match ignore rules", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "shioricode-workspace-tracked-gitignore-",
          git: true,
        });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("excludes .convex in non-git workspaces", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-non-git-convex-" });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("deduplicates concurrent index builds for the same cwd", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-concurrent-build-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("limits concurrent directory reads while walking the filesystem", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-read-concurrency-" });
  ...
  describe("listDirectory", () => {
    it.effect("lists a directory while filtering generated clutter", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "shioricode-workspace-list-directory-" });
        yield* writeTextFile(cwd, "src/index.ts", "export {};");
        yield* writeTextFile(cwd, "README.md", "# readme\n");
        yield* writeTextFile(cwd, ".DS_Store", "ignored");
    ...
    it.effect("filters gitignored entries from nested directory listings", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "shioricode-workspace-list-directory-gitignore-",
          git: true,
        });
        yield* writeTextFile(cwd, ".gitignore", "src/ignored.ts\nsrc/generated/\n");
        yield* writeTextFile(cwd, "src/keep.ts", "export const keep = true;\n");
        yield* writeTextFile(cwd, "src/ignored.ts", "export const ignored = true;\n");
        yield* writeTextFile(cwd, "src/generated/output.ts", "export const generated = true;\n");

        const result = yield* listWorkspaceDirectory({ cwd, relativePath: "src" });

        expect(result.directoryPath).toBe("src");
        expect(result.truncated).toBe(false);
        expect(result.entries).toEqual([{ path: "src/keep.ts", kind: "file", parentPath: "src" }]);
      }),
    );
  });
});
