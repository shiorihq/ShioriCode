...
function makeToolCallState(
  ...
  const summary = normalizedTitle ?? command ?? title ?? kind ?? "Tool";
  const detail = fallbackDetail && fallbackDetail !== summary ? fallbackDetail : undefined;
...
export function parseSessionUpdateEvent(params: EffectAcpSchema.SessionNotification): {
  ...
  switch (upd.sessionUpdate) {
    ...
    case "agent_thought_chunk": {
      // Surface model reasoning/thinking on the dedicated reasoning stream so it
      // is not silently dropped (ACP agents like Gemini and Cursor emit these).
      if (upd.content.type === "text" && upd.content.text.length > 0) {
        events.push({
          _tag: "ContentDelta",
          text: upd.content.text,
    ...
    default:
      // Remaining session/update variants are intentionally not projected yet:
      // available_commands_update, config_option_update, session_info_update,
      // and user_message_chunk. These require new orchestration event types and
      // UI plumbing rather than a content/tool projection; revisit when surfaced.
      break;
  }
