...
it.effect(
  "keeps a running turn open across assistant message completion until the session settles",
  () =>
    Effect.gen(function* () {
      const threadId = ThreadId.makeUnsafe("thread-turn-settle");
      const turnId = TurnId.makeUnsafe("turn-turn-settle");
      const startedAt = "2026-02-26T15:00:00.000Z";
      const assistantAt = "2026-02-26T15:00:05.000Z";
      const settledAt = "2026-02-26T15:00:12.000Z";
      const { dbPath } = yield* ServerConfig;
      const persistenceLayer = makeSqlitePersistenceLive(dbPath);
      ...
      yield* Effect.gen(function* () {
        ...
        yield* appendAndProject({
          type: "project.created",
          eventId: EventId.makeUnsafe("evt-turn-settle-1"),
          aggregateKind: "project",
          aggregateId: ProjectId.makeUnsafe("project-turn-settle"),
          occurredAt: startedAt,
          commandId: CommandId.makeUnsafe("cmd-turn-settle-1"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-turn-settle-1"),
          metadata: {},
          payload: {
            projectId: ProjectId.makeUnsafe("project-turn-settle"),
            title: "Project Turn Settle",
            workspaceRoot: "/tmp/project-turn-settle",
            defaultModelSelection: null,
            scripts: [],
            createdAt: startedAt,
            updatedAt: startedAt,
        ...
        yield* appendAndProject({
          type: "thread.created",
          eventId: EventId.makeUnsafe("evt-turn-settle-2"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: startedAt,
          commandId: CommandId.makeUnsafe("cmd-turn-settle-2"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-turn-settle-2"),
          metadata: {},
          payload: {
            threadId,
            projectId: ProjectId.makeUnsafe("project-turn-settle"),
            title: "Thread Turn Settle",
            modelSelection: {
              provider: "codex",
              model: "gpt-5.4",
            },
        ...
        yield* appendAndProject({
          type: "thread.session-set",
          eventId: EventId.makeUnsafe("evt-turn-settle-3"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: startedAt,
          commandId: CommandId.makeUnsafe("cmd-turn-settle-3"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-turn-settle-3"),
          metadata: {},
          payload: {
            threadId,
            session: {
        ...
        yield* appendAndProject({
          type: "thread.message-sent",
          eventId: EventId.makeUnsafe("evt-turn-settle-4"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: assistantAt,
          commandId: CommandId.makeUnsafe("cmd-turn-settle-4"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-turn-settle-4"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.makeUnsafe("assistant-turn-settle"),
            role: "assistant",
            text: "status update",
            turnId,
            streaming: false,
        ...
        assert.deepEqual(runningRows, [
          {
            state: "running",
            completedAt: null,
            assistantMessageId: "assistant-turn-settle",
          },
        ]);

        yield* appendAndProject({
          type: "thread.session-set",
          eventId: EventId.makeUnsafe("evt-turn-settle-5"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: settledAt,
          commandId: CommandId.makeUnsafe("cmd-turn-settle-5"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-turn-settle-5"),
          metadata: {},
          payload: {
            threadId,
            session: {
      ...
      }).pipe(Effect.provide(Layer.mergeAll(projectionLayer, eventStoreLayer, persistenceLayer)));
    }).pipe(
      Effect.provide(
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "t3-projection-pipeline-turn-settle-",
          }),
          NodeServices.layer,
        ),
      ),
