...
type BufferedAssistantTextEntry = {
  text: string;
...
function commentaryAssistantPhaseFromEvent(event: ProviderRuntimeEvent): string | undefined {
  const payloadRecord = asObject(event.payload);
  const payloadData = asObject(payloadRecord?.data);
  ...
  return asString(payloadItem?.phase) ?? asString(rawItem?.phase);
}

function commentaryAssistantTextFromEvent(event: ProviderRuntimeEvent): string | undefined {
...
function normalizeAssistantDelta(input: {
  existingText: string;
  delta: string;
  previousContentIndex?: number | undefined;
  nextContentIndex?: number | undefined;
}): string {
  const { existingText, delta, previousContentIndex, nextContentIndex } = input;
  if (
    previousContentIndex === undefined ||
    nextContentIndex === undefined ||
    previousContentIndex === nextContentIndex ||
  ...
  const existingNewlines = countTrailingNewlines(existingText);
  const deltaNewlines = countLeadingNewlines(delta);
  return `${"\n".repeat(Math.max(0, 2 - (existingNewlines + deltaNewlines)))}${delta}`;
}
...
function isReasoningStreamKind(
  value: string | undefined,
...
function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "content.delta": {
      if (!isReasoningStreamKind(event.payload.streamKind) || event.payload.delta.length === 0) {
        break;
      }
...
const make = Effect.fn("make")(function* () {
  ...
  const bufferedAssistantTextByMessageId = yield* Cache.make<MessageId, BufferedAssistantTextEntry>(
    {
      capacity: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_CACHE_CAPACITY,
      timeToLive: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_TTL,
      lookup: () => Effect.succeed({ text: "", lastContentIndex: undefined }),
    },
  );

  const commentaryAssistantItemsByItemId = yield* Cache.make<string, boolean>({
    capacity: COMMENTARY_ASSISTANT_ITEMS_BY_ID_CACHE_CAPACITY,
    timeToLive: COMMENTARY_ASSISTANT_ITEMS_BY_ID_TTL,
  ...
  const rememberCommentaryAssistantItemId = (itemId: string | undefined) =>
    itemId ? Cache.set(commentaryAssistantItemsByItemId, itemId, true) : Effect.void;

  const forgetCommentaryAssistantItemId = (itemId: string | undefined) =>
    itemId ? Cache.invalidate(commentaryAssistantItemsByItemId, itemId) : Effect.void;

  const isCommentaryAssistantItemId = (itemId: string | undefined) =>
    itemId ? Cache.get(commentaryAssistantItemsByItemId, itemId) : Effect.succeed(false);
    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap(
        Effect.fn("appendBufferedAssistantText")(function* (existingText) {
          const existingEntry = Option.getOrUndefined(existingText);
          const priorText = existingEntry?.text.length ? existingEntry.text : existingRenderedText;
          const normalizeInput: {
            ...
            existingText: priorText,
            delta,
          };
          if (existingEntry?.lastContentIndex !== undefined) {
            normalizeInput.previousContentIndex = existingEntry.lastContentIndex;
          }
          if (contentIndex !== undefined) {
            normalizeInput.nextContentIndex = contentIndex;
          }
          const nextDelta = normalizeAssistantDelta(normalizeInput);
          const nextLastContentIndex = contentIndex ?? existingEntry?.lastContentIndex;
          const nextEntry: BufferedAssistantTextEntry = {
            text: `${existingEntry?.text ?? ""}${nextDelta}`,
            ...(nextLastContentIndex !== undefined
              ? { lastContentIndex: nextLastContentIndex }
              : {}),
          };
          ...
          yield* Cache.set(bufferedAssistantTextByMessageId, messageId, {
            text: "",
            ...(nextEntry.lastContentIndex !== undefined
              ? { lastContentIndex: nextEntry.lastContentIndex }
              : {}),
          });
    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Cache.invalidate(bufferedAssistantTextByMessageId, messageId).pipe(
          Effect.as(
            Option.getOrElse(existingText, () => ({ text: "", lastContentIndex: undefined })).text,
          ),
        ),
      ),
    );
  ...
  const recordAssistantContentIndex = (messageId: MessageId, contentIndex: number | undefined) =>
    contentIndex === undefined
      ? Effect.void
      : Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
          Effect.flatMap((existingEntry) => {
            const entry = Option.getOrElse(existingEntry, () => ({
              text: "",
              lastContentIndex: undefined,
            }));
            return Cache.set(bufferedAssistantTextByMessageId, messageId, {
              text: entry.text,
  ...
  const processRuntimeEvent = Effect.fn("processRuntimeEvent")(function* (
    ...
    const commentaryAssistantItemId =
      event.itemId && commentaryAssistantPhaseFromEvent(event) === "commentary"
        ? String(event.itemId)
        : undefined;
    if (event.type === "item.started" && commentaryAssistantItemId) {
      yield* rememberCommentaryAssistantItemId(commentaryAssistantItemId);
    }

    const isCommentaryAssistantStream =
      event.type === "content.delta" &&
      event.payload.streamKind === "assistant_text" &&
      (yield* isCommentaryAssistantItemId(event.itemId ? String(event.itemId) : undefined));

    const assistantDelta =
      event.type === "content.delta" &&
      event.payload.streamKind === "assistant_text" &&
      !isCommentaryAssistantStream
        ? event.payload.delta
        : undefined;
    ...
    if (assistantDelta && assistantDelta.length > 0) {
      ...
      const assistantContentIndex =
        event.type === "content.delta" && typeof event.payload.contentIndex === "number"
          ? event.payload.contentIndex
          : undefined;
      const turnId = toTurnId(event.turnId);
      const existingAssistantText =
        thread.messages.find((entry) => entry.id === assistantMessageId)?.text ?? "";
      if (turnId) {
      ...
      if (assistantDeliveryMode === "buffered") {
        ...
        const assistantState = yield* Cache.getOption(
        ...
        const normalizeInput: {
          ...
          existingText: existingAssistantText,
          delta: assistantDelta,
        };
        const previousContentIndex = Option.getOrUndefined(assistantState)?.lastContentIndex;
        if (previousContentIndex !== undefined) {
          normalizeInput.previousContentIndex = previousContentIndex;
        }
        if (assistantContentIndex !== undefined) {
          normalizeInput.nextContentIndex = assistantContentIndex;
        }
        const normalizedDelta = normalizeAssistantDelta(normalizeInput);
        yield* orchestrationEngine.dispatch({
          type: "thread.message.assistant.delta",
        ...
        yield* recordAssistantContentIndex(assistantMessageId, assistantContentIndex);
      }
    }
    ...
    if (assistantCompletion) {
      const isCommentaryAssistantCompletion =
        commentaryAssistantPhaseFromEvent(event) === "commentary" ||
        (yield* isCommentaryAssistantItemId(event.itemId ? String(event.itemId) : undefined));
      if (isCommentaryAssistantCompletion) {
        yield* forgetCommentaryAssistantItemId(event.itemId ? String(event.itemId) : undefined);
      } else {
