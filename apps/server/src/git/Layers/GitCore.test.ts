...
import { GitCommandError } from "contracts";
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "shioricode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "shioricode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "shioricode/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "shioricode/tmp-working",
          newBranch: "shioricode/feat/session",
        });

        expect(renamed.branch).toBe("shioricode/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "shioricode/feat/session")).toBe(
          true,
        );
        expect(
          branches.branches.some((branch) => branch.name === "shioricode/feat/session-1"),
        ).toBe(true);
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("shioricode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "shioricode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "shioricode/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "shioricode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "shioricode/tmp-working" });
        ...
        expect(renamed.branch).toBe("shioricode/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/shioricode.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/shioricode.git/",
          });
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, [
            "checkout",
            "-b",
            "shioricode/pr-488/statemachine",
            "--track",
