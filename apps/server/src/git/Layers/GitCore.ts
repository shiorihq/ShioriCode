...
import { GitCommandError, type GitBranch } from "contracts";
import { dedupeRemoteBranchesWithLocalMatches } from "shared/git";
import {
  GitCore,
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `shioricode-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitCore = Effect.fn("makeGitCore")(function* (options?: {
  ...
  const statusDetails: GitCoreShape["statusDetails"] = Effect.fn("statusDetails")(
