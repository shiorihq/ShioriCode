...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  ...
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "shioricode-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__SHIORICODE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__SHIORICODE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "shioricode-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGenerationLive", (it) => {
  ...
  it.effect("defaults git text generation codex effort to low and skips repo trust checks", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
