...
import type { GitActionProgressEvent, ModelSelection } from "contracts";

import { GitCommandError, GitHubCliError, TextGenerationError } from "contracts";
...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("shioricode-git-remote-");
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("shioricode-git-manager-non-repo-");
      const { manager } = yield* makeManager();
              ...
              JSON.stringify([
                {
                  ...
                  url: "https://github.com/pingdotgg/shioricode/pull/1661",
                  baseRefName: "main",
                  headRefName: "main",
                  state: "OPEN",
                  ...
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/shioricode",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "shioricode/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.branch).toBe("shioricode/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "shioricode/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* runGit(repoDir, [
          "config",
          "remote.fork-seed.url",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              "shioricode/pr-142/statemachine": JSON.stringify([]),
  ...
  it.effect("creates cross-repo PRs with the fork owner selector and default base branch", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "shioricode/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* runGit(repoDir, [
        "config",
        "remote.fork-seed.url",
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/pingdotgg/shioricode/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("shioricode/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("shioricode/pr-92/main");
