...
import { GitManagerError } from "contracts";
import {
  GitManager,
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `shioricode/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `shioricode-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
