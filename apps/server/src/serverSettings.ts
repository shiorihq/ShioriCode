...
import { type DeepPartial, deepMerge } from "shared/Struct";
import { fromLenientJson } from "shared/schemaJson";
...
function resolveEffectiveServerSettings(settings: ServerSettings): ServerSettings {
  ...
  return {
    ...nextSettings,
    defaultModelSelection: resolveModelSelectionProvider(
      nextSettings,
      nextSettings.defaultModelSelection,
      (provider) => DEFAULT_MODEL_BY_PROVIDER[provider],
    ),
    textGenerationModelSelection: resolveModelSelectionProvider(
      nextSettings,
...
const makeServerSettings = Effect.gen(function* () {
  ...
  return {
    ...
    getSettings: getSettingsFromCache.pipe(Effect.map(resolveEffectiveServerSettings)),
    updateSettings: (patch) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          return resolveEffectiveServerSettings(next);
        }),
      ),
    get streamChanges() {
      return Stream.fromPubSub(changesPubSub).pipe(Stream.map(resolveEffectiveServerSettings));
    },
  } satisfies ServerSettingsShape;
});
